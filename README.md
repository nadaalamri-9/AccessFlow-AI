...
## Run
Open `AccessFlow_AI_Capstone.ipynb` in Google Colab and choose **Runtime → Run all**. The setup cell clones this repository when needed and starts the zero-key backend automatically.
...
The default backend is a course-style zero-key HTTP simulator so the notebook can be rerun without credentials. Its HTTP contracts, structured outputs, usage accounting, prompt-cache evidence, routing, guardrails, retry/repair flow, tools, evaluation, and fault handling are executable evidence about this application harness. The reported quality numbers are not claims about external commercial model providers.
